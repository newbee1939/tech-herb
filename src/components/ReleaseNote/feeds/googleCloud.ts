...
export const getGoogleCloudReleaseNote = async () => {
  ...
  try {
    const googleCloudReleaseNotes = (
      await (
        await fetch(
          `https://api.rss2json.com/v1/api.json?rss_url=https://cloud.google.com/feeds/gcp-release-notes.xml&api_key=${import.meta.env.RSS_2_JSON_API_KEY}`,
        )
      ).json()
    ).items;
    const mappedGoogleCloudReleaseNotesPromises = googleCloudReleaseNotes
      .filter((releaseNote: GoogleCloudReleaseNote) => {
    ...
    mappedGoogleCloudReleaseNotes = await Promise.all(
      mappedGoogleCloudReleaseNotesPromises,
    );
  } catch (e) {
    console.error(e);
