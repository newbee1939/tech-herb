...
export const getQiitaMedium = async () => {
  ...
  try {
    const latestQiitaArticles = (
      await (
        await fetch(
          `https://api.rss2json.com/v1/api.json?rss_url=https://qiita.com/popular-items/feed&api_key=${import.meta.env.RSS_2_JSON_API_KEY}`,
        )
      ).json()
    ).items;
    const slicedQiitaArticlesPromises = latestQiitaArticles
      .slice(0, articleLimit)
      .map(async (article: QiitaArticle) => {
