...
export const getPublickeyMedium = async () => {
    ...
    try {
        ...
        const slicedPublickeyArticlesPromises = latestPublickeyArticles.filter((article: PublicKeyArticle) => {
            // PR用の記事は除外
        ...
        }).slice(0, articleLimit).map(async (article: PublicKeyArticle) => {
        ...
        slicedPublickeyArticles = await Promise.all(slicedPublickeyArticlesPromises);
    } catch(e) {
        console.error(e);
