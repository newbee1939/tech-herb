...
export const getItMediaMedium = async () => {
  ...
  try {
    const latestItMediaArticles = (
      await (
        await fetch(
          `https://api.rss2json.com/v1/api.json?rss_url=https://rss.itmedia.co.jp/rss/2.0/topstory.xml&api_key=${import.meta.env.RSS_2_JSON_API_KEY}`,
        )
      ).json()
    ).items;
    const slicedItMediaArticlesPromises = latestItMediaArticles
      .filter((article: ItMediaArticle) => {
        const now = new Date();
        // 前日の朝5時を計算
      ...
      .map(async (article: ItMediaArticle) => {
